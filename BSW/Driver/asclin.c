...
//    /* 입력 받은 문자가 ‘1’ 이면, LED ON */
//    if (c == '1')
//    {
//        GPIO_SetLed(1, 1);
//        my_printf("LED ON!\n");
//    }
//    /* 입력 받은 문자가 ‘0’ 이면, LED OFF */
//    else if (c == '0')
//    {
//        GPIO_SetLed(1, 0);
//        my_printf("LED OFF!\n");
...
void Asclin0_InitUart (void)
{
    ...
    MODULE_P14.IOCR0.B.PC0 = 0x12; // 0001 0010
...
//    /* set numerator and denominator for 38400 baudrate */
//    unsigned int numerator = 192;
//    unsigned int denominator = 3125;
    ...
    IfxScuWdt_setCpuEndinit(IfxScuWdt_getGlobalEndinitPassword());
    /* read back for activating module */
    (void)MODULE_ASCLIN1.CLC.U; // error 확인용. clc enable 반영 여부 확인 위함
    ...
    MODULE_ASCLIN1.BITCON.U = ( 9 << 0) /* PRESCALER: 10 */
                            ...
                            | (1u << 31); /* SM: 3 samples per bit */
    /* TXFIFO 설정 */
    MODULE_ASCLIN1.TXFIFOCON.U = (1 << 6) // INW:1 byte
                               | (1 << 1) /* ENO */
                               | (1 << 0); /* FLUSH */
    /* RXFIFO 설정 */
